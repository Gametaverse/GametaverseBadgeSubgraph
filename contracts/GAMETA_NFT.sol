...
contract GMNFT is ERC721Enumerable, Ownable{
    ...
    mapping(address => uint256) public referNumber; //指定地址推荐人数
    mapping(address => bool) public claimStatus; //指定地址领取NFT的状态
    uint256 public next_tokenID;
    event ClaimNFT(address _user, uint256 _tokenID, address _referrer);
    event WhitelistAdded(address indexed user);
    event WhitelistRemoved(address indexed user);
    constructor() ERC721("Gametaverse Ambassador", "GAMETA") {}
    
    //领取NFT
    function claimNFT(address _referrer)  public  returns(uint256){
        require(!claimStatus[msg.sender], "The user already claimed.");
        require(msg.sender !=  _referrer, "The referrer is caller.");
        uint256 _next_tokenID = next_tokenID;
        _safeMint(msg.sender, _next_tokenID);
        if (_referrer != address(0)){
            referNumber[_referrer] = referNumber[_referrer].add(1);
        }
        claimStatus[msg.sender] = true;
        next_tokenID++;
        emit ClaimNFT(msg.sender, _next_tokenID, _referrer);
